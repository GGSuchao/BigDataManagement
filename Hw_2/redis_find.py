    ...
    for key in keys:
        if str(r.type(key)) == 'hash':
            ...
            for i in range(field_num):
                p_name = 'P' + str(i)
                s_name = 'S' + str(i)
                if (r.hget(key, p_name) == 'isLeaderOf'):
                    s_list.append(r.hget(key, s_name))
    for key in keys:
        if str(r.type(key)) == 'hash':
            field_num = r.hlen(key) / 2
            for i in range(field_num):
                p_name = 'P' + str(i)
                s_name = 'S' + str(i)
                if (r.hget(key, p_name) == 'actedIn' and r.hget(key, s_name) in s_list):
                    res_list.append(r.hget(key, s_name))
    for rec in list(set(res_list)):
        print(rec)
    time2 = time.time()
    print("Use %.7f seconds" % (time2 - time1))
